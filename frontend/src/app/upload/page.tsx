...
export default function UploadPage() {
  ...
  // File System Access API 우선 시도 → 실패 시 input 클릭
  const pickImage = async () => {
    // @ts-ignore: 브라우저 지원 여부 런타임 체크
    const canFS = typeof window !== 'undefined' && typeof window.showOpenFilePicker === 'function';
    if (canFS) {
      try {
        // @ts-ignore
      ...
      <main className="p-6">
        ...
        >
          <input
            ref={fileInputRef}
            type="file"
            accept="image/*"     // ★ capture 속성 완전히 제거 (네이버 인앱에서 카메라 강제 방지)
            className="hidden"
            onChange={onChange}
          />
          ...
          {imagePreview ? (
            ...
            <div className="flex flex-col items-center justify-center text-gray-500">
              <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48"
                ...
                <line x1="16" x2="22" y1="5" y2="5"></line><line x1="19" x2="19" y1="2" y2="8"></line>
                <circle cx="9" cy="9" r="2"></circle>
                <path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"></path>
              </svg>
              <p className="mt-2 font-semibold text-center">사진을 추가하려면<br />여기를 탭하세요.</p>
